...
def _normalize_settings(raw: Any) -> dict:
    ...
    def _norm_codes(values: Any) -> list[str]:
        if not isinstance(values, list):
            return []
        return [str(x).strip().upper() for x in values if str(x).strip()]

    def _norm_regions(values: Any) -> list[str]:
        if not isinstance(values, list):
            return []
    ...
    fx_raw = raw.get("fx_rate")
    if fx_raw in (None, ""):
        fx_raw = raw.get("roe_rate")

    return {
        "allowed_countries": _norm_codes(raw.get("allowed_countries")),
        "allowed_regions": _norm_regions(raw.get("allowed_regions")),
        "blocked_countries": _norm_codes(raw.get("blocked_countries")),
        "blocked_regions": _norm_regions(raw.get("blocked_regions")),
        "fx_rate": _num(fx_raw, 0.0),
...
def load_config() -> dict:
    ...
    data = sb_load_or_seed(
        doc_key="esim_oasis_config",
        default={"accounts": [], "active_account_id": "", "settings": _normalize_settings({}), "fx_history": []},
        local_loader=_load_local,
    )
    if not isinstance(data, dict):
        data = {"accounts": [], "active_account_id": "", "settings": _normalize_settings({}), "fx_history": []}
...
def save_config(cfg: dict) -> dict:
    ...
    sb_save(doc_key="esim_oasis_config", value=cfg, local_saver=_save_local)
    return cfg
