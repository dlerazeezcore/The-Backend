...
from backend.flights.wings.services.normalize import normalize_priced_itineraries
...
from backend.gateway.permissions_store import _ota_policy
...
async def availability(req: AvailabilityRequest):
    ...
    async def _roundtrip_price_map() -> dict:
        """Build lookups for roundtrip totals.

        Returns:
          {
            "return_map": {ret_sig: price_info},
            "pair_map": {"out_sig||ret_sig": price_info},
          }
        """
        if req.trip_type != "roundtrip" or not req.return_date:
            return {"return_map": {}, "pair_map": {}}
        ...
        for pi in pis:
            try:
                ...
                segs_out = (odo_list[0] or {}).get("flightSegment") or []
                if not isinstance(segs_out, list):
                    segs_out = [segs_out]
                if not segs_out:
                    continue
                ...
                out_sig = _seg_sig(segs_out[0] or {})
                ...
                info = {"currency": ccy, "amount": amt_disp, "amount_raw": amt_f}
                # Legacy fallback map keyed by return segment only.
                if sig not in out_return:
                    out_return[sig] = info
                # Precise map keyed by outbound+return pair.
                out_pair[f"{out_sig}||{sig}"] = info
            except Exception:
                continue

        return {"return_map": out_return, "pair_map": out_pair}
    ...
    try:
        ...
        if req.trip_type == "roundtrip" and req.return_date:
            ...
            try:
                rt_map_data = await asyncio.wait_for(rt_map_task, timeout=8)
                if isinstance(rt_map_data, dict):
                    rt_map = rt_map_data.get("return_map") or {}
                    rt_pair_map = rt_map_data.get("pair_map") or {}
            except Exception:
                rt_map = {}
                rt_pair_map = {}
            if rt_map:
            ...
            if rt_pair_map:
                if not isinstance(meta, dict):
                    meta = {}
                meta["roundtrip_pair_map"] = rt_pair_map
        else:
